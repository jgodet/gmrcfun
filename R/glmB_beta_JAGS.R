...
glmB_beta_JAGS<-function(formul,data=NULL,Niter=105000,Nburnin=5000,Nthin=5,Prior=NULL,Seuilmonit=NULL,Graph=TRUE){
...
#####################################################
###	verifier si reponse 01	 	#####
#####################################################

message_Erreur01<-paste("Erreur:\n
La variable reponse n'appartient pas a l'intervalle ]0;1[ 
Vous pouvez la transformer en utilisant la commande\n
\n cs01(", colnames(variables_Matrice)[1], ", min, max ) \n
dont le role est de reduire votre score compris entre min et max dans ]0;1[ \n\n" )

if(min(variable_reponse,na.rm=TRUE)>0 & max(variable_reponse,na.rm=TRUE)<1){Variable_entre01<-TRUE}else{ 
cat(message_Erreur01)
cat("\n")
stop("",call. = FALSE)
}
...
for( i in 1 : N ) {Y[i]~dbeta(a[i],b[i]) 

a[i]<-mu[i]*gamma
b[i]<-(1-mu[i])*gamma

for( j in 1 : nbcoefx ){monome[i,j]		<- designmat[i,j]*beta[j]}                                                                                                 
                  	logit(mu[i]) 	<- sum(monome[i,])   } 
...
initi<-list(list(beta=rep(0.2,nombre_coef_burtee),gamma=0.2))
...
##################################################################################
cat("---------------------------");cat("\n")
cat("REGRESSION BETA BAYESIENNE");cat("\n")
cat("---------------------------");cat("\n")
cat("---Avertissement: Bien que dans la fonction glmB, beta n'est pas un GLM---");cat("\n")
cat("---------------------------");cat("\n")
