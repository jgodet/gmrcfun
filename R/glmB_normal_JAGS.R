...
glmB_normal_JAGS<-function(formul,data=NULL,Niter=105000,Nburnin=5000,Nthin=5,Prior=NULL,Seuilmonit=NULL,Graph=TRUE){
...
if(is.null(Prior)){Priors<-rbind(rep(0,nombre_coef_burtee),rep(0.001,nombre_coef_burtee))}else{Priors<-Prior}
...
Par exemple, \n Le prior peu informatif pour chaque coef est une N(0,0.001), \n La matrice par defaut est:\n")

Priors_non_informatifs<-	rbind(	rep(0,nombre_coef_burtee),
									rep(0.001,nombre_coef_burtee))
...
parametre       <-c("beta","Pr_beta_pos")
initi<-list(list(beta=rep(0.2,nombre_coef_burtee	),tau=0.1))
...
#####################################################################################
###################        Diagnostics Normalite	 ##############################
#####################################################################################

coefficients_moy	<-RET[1:nombre_coef_burtee,1]
predictions		<-design_mat_burtee%*%coefficients_moy
residus		<-variable_reponse-predictions
if(Graph){
par(mfrow=c(2,2))
plot(residus,pch=16)	;abline(0,0)
hist(residus)
qqnorm(residus); qqline(residus)
par(mfrow=c(1,1))
}
if(length(variable_reponse) >4999 | length(variable_reponse) <4){
Shapiro<-"NA (Effectifs)"}else{Shapiro<-round(shapiro.test(residus)$p.value,5) }
...
cat("\n");cat("\n")
cat("---------------------------");cat("\n")
cat("REGRESSION GAUSIENNE BAYESIENNE");cat("\n")
cat("---------------------------");cat("\n");cat("\n")
cat("---------------------------");cat("\n")
titre<-paste("---	Graphique: Analyse des residus (Shapiro:",Shapiro,")--------")
cat(titre);cat("\n")
cat("---------------------------");cat("\n")
